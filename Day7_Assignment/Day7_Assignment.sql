...
*/
    ...
    order_date,
    freight,
    LAG(freight, 1, NULL) OVER (PARTITION BY customer_id ORDER BY order_date) AS previous_freight,
    LEAD(freight, 1, NULL) OVER (PARTITION BY customer_id ORDER BY order_date) AS next_freight
FROM
    orders
ORDER BY
...
*/
	WITH ProductPriceCategories AS (
    SELECT
        product_id,
        product_name,
        unit_price,
